...
def install_dataset_explorer(CONFIG):
    ...
    # Use canonical dataset-explorer location inside the repo
    ...
    if not os.path.exists(dataset_explorer_dir):
        ...
        return
    print(f"Using dataset-explorer directory: {dataset_explorer_dir}")
    ...
    try:
        ...
        print("BUILD: Running npm script: build-mini-node")
        build_result = cmd(
        ...
        if build_result != 0:
            ...
            print("BUILD: MAKE SURE DOCKER IS INSTALLED AND RUNNING")
            return
        ...
        cmd(f"sudo mkdir -p {host_ui_dir}")
        cmd(f"sudo chmod 0777 {host_ui_dir}")

        # Create tarball of build and extract directly into host path
        print("COPY: COPYING build/ DIRECTLY TO HOST_UI_DIR (no /tmp tarball)")
        # Ensure the destination exists
        cmd(f"sudo mkdir -p {host_ui_dir}")
        # Remove previous files (if any)
        print("COPY: REMOVING EXISTING FILES IN HOST_UI_DIR")
        cmd(f"sudo rm -rf {host_ui_dir}/*", exit_on_error=False)

        # Try a direct copy first
        print(f"COPY: COPYING build/ -> {host_ui_dir} (direct cp)")
        copy_ret = cmd(f"sudo cp -a build/. {host_ui_dir}/", exit_on_error=False)

        if copy_ret != 0:
            print("COPY: direct cp failed, trying tar-pipe fallback (no /tmp file)")
            # Fallback: tar from build and extract to destination via a pipe (avoids writing a tar file to /tmp)
            pipe_cmd = f"sudo bash -c 'cd build && tar -czf - .' | sudo tar -xzf - -C {host_ui_dir}"
            pipe_ret = cmd(pipe_cmd, exit_on_error=False)
            if pipe_ret != 0:
                print(f"COPY: ERROR - both direct copy and tar-pipe failed (rc={pipe_ret}). Aborting copy step.")
                return
            else:
                print("COPY: tar-pipe fallback succeeded")
        else:
            print("COPY: direct cp succeeded")

        print("COPY: SETTING PERMISSIONS ON HOST_UI_DIR")
        cmd(f"sudo chmod -R 0777 {host_ui_dir}")

        print("COPY: FILES COPIED TO HOST PATH SUCCESSFULLY")

        # Ensure permissions so the container can read the files
        print(f"COPY: SETTING PERMISSIONS ON {host_ui_dir}")
        cmd(f"sudo chmod -R 0777 {host_ui_dir}")

        print("COPY: FILES COPIED TO HOST PATH SUCCESSFULLY")
        
        # Restart dataset-service pod to pick up new files
        print("\nSERVICE: RESTARTING dataset-service TO LOAD NEW UI FILES...")
        print("SERVICE: DELETING PODS WITH LABEL app=dataset-service-backend -n dataset-service")
        cmd("minikube kubectl -- delete pod -l app=dataset-service-backend -n dataset-service")
        cmd("sleep 10")
        print("SERVICE: WAITING FOR dataset-service BACKEND PODS TO BECOME READY")
        cmd("minikube kubectl -- wait --for=condition=ready pod -l app=dataset-service-backend -n dataset-service --timeout=120s || true")

        print("SERVICE: RESTART COMPLETE (pods signaled to restart)")
...
# K8S_TOKEN LA SEGUNDA VEZ YA NO LO VA A REEMPLAZAR
    ...
    # Create the required directories on the host (use configured host_path)
    print(f"\n Creating required directories...")
    host_path = getattr(CONFIG, 'host_path', '/home/ubuntu/minikube-data')
    # normalize and remove trailing slash if present
    host_path = host_path.rstrip('/')
    users_dir = f"{host_path}/data/homes/users"
    shared_dir = f"{host_path}/data/homes/shared-folder"
    homes_dir = f"{host_path}/data/homes"
    cmd(f"sudo mkdir -p {users_dir}")
    cmd(f"sudo mkdir -p {shared_dir}")
    cmd(f"sudo chmod -R 0777 {homes_dir}")
    print(f" Directories created: {homes_dir}")
