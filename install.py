...
def install_dataset_explorer(CONFIG):
    ...
    try:
        ...
        print("\nCopying files to host path /home/ubuntu/minikube-data (host-mounted to /var/hostpath-provisioner)...")

        host_ui_dir = "/home/ubuntu/minikube-data/dataset-service/ui"
        ...
        cmd(f"sudo rm -rf {host_ui_dir}/*")
        cmd(f"sudo tar -xzf /tmp/dataset-explorer-build.tar.gz -C {host_ui_dir}")
...
def install_kubeapps(CONFIG, client_kubernetes_secret: str):
    ...
    try:
        ...
        if use_gateway_api:
            ...
            # LEGACY: Apply traditional Ingress for Kubeapps
            print(f"\n Configuring Ingress for Kubeapps...")
            ingress_file = "kubeapps-ingress.yaml"
            if os.path.exists(ingress_file):
                # Update ingress with correct domain
                with open(ingress_file, 'r') as f:
                    ingress_content = f.read()

                # Replace host in rules
                ingress_content = re.sub(r'host:\s+[a-zA-Z0-9.-]+', f'host: {CONFIG.public_domain}', ingress_content)

                # Replace hosts in tls section
                ingress_content = re.sub(
                    r'(-\s+hosts:\s*\n\s+-\s+)[a-zA-Z0-9.-]+',
                    rf'\1{CONFIG.public_domain}',
                    ingress_content
                )

                # Replace secretName with domain (replacing dots with dashes)
                secret_name = CONFIG.public_domain.replace('.', '-') + '-tls'
                ingress_content = re.sub(r'secretName:\s+[a-zA-Z0-9.-]+', f'secretName: {secret_name}', ingress_content)

                # Write to temporary file
                temp_ingress = "/tmp/kubeapps-ingress-updated.yaml"
                with open(temp_ingress, 'w') as f:
                    f.write(ingress_content)

                # Apply the updated ingress
                cmd(f"minikube kubectl -- apply -f {temp_ingress}")
                print(f" Ingress applied for Kubeapps at https://{CONFIG.public_domain}/apps")
                print(f"   - Host: {CONFIG.public_domain}")
                print(f"   - TLS Secret: {secret_name}")
            else:
